...
前述の通り、異なる[変更の種類](change-types.md)によって異なるプロセスが適用されます。以下は、[機能的変更(Functional changes)](change-types.md/#functional-changes)に関するプロセスの概要であり、コミュニティがどのように提案、レビュー、テスト、採用を行うかを示しています。
