...
GTFS Reference フォーマットは、公共交通システムの現在のニーズに対応するために進化するにつれて、その機能がますます複雑になることがあります。**GTFS の機能(GTFS Features)** は、GTFS Reference フォーマットによって実現される機能を明確かつ決定的に説明することを目的としています。これにより、交通事業者、ベンダー、利用者、研究者が GTFS の機能を理解し、「GTFS で何ができるのか？」という問いに答えることができます。

以下の機能グループでは、それぞれの機能の目的と、それに関連するファイルおよびフィールドを説明し、特定の機能をサポートするためにどのデータが必要かを理解できるようにしています。

## 基本要素 {: #base}

これらの基本的な機能は、GTFS フィードの中核を形成します。これらは、公共交通サービスを表現するために必要な最小限の要素です。
...
-   :material-escalator:{ .lg .middle } __階層(レベル)__
...
-   :material-escalator:{ .lg .middle } __構内通路の案内標識__

    構内通路に関連する駅構内の案内標識を伝達します。
