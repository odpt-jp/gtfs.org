...
- [Catenary Maps](https://catenarymaps.org) - RustとSvelteで書かれた、リアルタイムおよびスケジュール対応のグローバルな公共交通マップおよびナビゲーションソフトウェアです。
...
- [Google I/O Transport Tracker](https://github.com/googlemaps/transport-tracker) - Google I/Oカンファレンスのシャトル到着時刻を表示します。オープンソースの[transport-trackerプロジェクト](https://github.com/googlemaps/transport-tracker)に基づいています。  
  注: 自身で実装する場合は、[Google Maps APIs Premium Planライセンス](https://developers.google.com/maps/pricing-and-plans/)が必要です。
...
- [Bustime](https://busti.me) - WebSocketによる更新を用いた公共交通のリアルタイム監視です。オープンソース版は[GitHub](https://github.com/norn/bustime)にあります。
...
- [Cadê Meu Busão](https://tarifazerobh.org/cade-meu-busao/) - ブラジル・ベロオリゾンテの公共交通バスをリアルタイムで追跡します。オープンソース版は[GitHub](https://github.com/tarifazero/monitoramento)にあります。
...
- [Citylines.co](https://www.citylines.co) - 交通システムを共同でマッピングするためのプラットフォームで、その歴史的変遷に重点を置いています。
...
- [CityMapper Webapp](https://citymapper.com/nyc) - 30以上の都市に対応した、旅程計画およびルート状況を提供する非常に洗練されたWebアプリ。
...
- [opentripplanner-client-library](https://github.com/CUTR-at-USF/opentripplanner-client-library) - Android、iOS、Web 向けに、OpenTripPlanner v2 サーバーから旅程計画、自転車レンタル情報、サーバーメタデータの API リクエストおよびレスポンス解析を行うための Kotlin マルチプラットフォームライブラリ。
...
- [Transit Display](http://transitdisplay.com/) - 複数の交通手段に対応し、リアルタイムの交通情報を表示するソフトウェアです。
- [Ualabee](https://ualabee.com/company/) - ユーザーの相互作用に重点を置いたコミュニティ主導の経路検索アプリで、ユーザーは異常の報告、写真のアップロード、交通データの編集、他の乗客とのチャットができます。
...
- [SkedGo の TripKit SDKs](https://developer.tripgo.com) - [SkedGo](https://skedgo.com) の TripGo API にアクセスするための Android、iOS、React 向けオープンソース SDK で、旅程(journey)計画用の UI コンポーネントを含みます。
...
- [MTA Frequency](http://www.tyleragreen.com/maps/new_york/) - [Transitland](https://transit.land/) を使用して構築された、ニューヨーク市の地下鉄およびバスの運行頻度の可視化です。
...
- [GTFS Viz 🚉](https://github.com/gabrielAHN/gtfs-viz) - [duckdb-wasm 🦆](https://duckdb.org/docs/api/wasm/overview.html) を使用して、バックエンドを持たずにクライアント側で大規模な GTFS データをブラウザ上で可視化する Web アプリです。
...
### 交通地図アグリゲーション {: #transit-map-aggregation}
 ...
 - [AllRailMap](https://www.allrailmap.com/) - OpenStreetMapデータを使用したもう一つの世界鉄道地図です。
 ...
 - [National Rail Network Map](https://www.arcgis.com/apps/mapviewer/index.html?webmap=96ec03e4fc8546bd8a864e39a2c3fc41) - 米国の鉄道路線（旅客および貨物線を含む）の範囲と所有権を示す地図です。
 ...
 - [Railway Stations](https://map.railway-stations.org/) - 世界中の鉄道駅の写真を集めたサイトです。
 ...
 - [China Railway Map](http://cnrail.geogv.org/enus/about) - 中国の旅客鉄道輸送システムの駅および路線情報を示すオンラインインタラクティブマップです。
...
- [Remix](http://getremix.com/) - 交通事業者がルートを簡単に計画できるウェブアプリです。
...
- [TBEST](https://tbest.org/) - TBEST（Transit Boardings Estimation and Simulation Tool）は、社会経済、土地利用、交通ネットワークデータを統合し、シナリオベースの交通需要推計および分析を行う多面的な GIS ベースのモデリング・計画・分析ツールを開発する取り組みです。フロリダ州運輸局が資金提供しています。無料で利用できますが、オープンソースではありません。
