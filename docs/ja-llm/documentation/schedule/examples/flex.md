...
Zone1 から Zone3 への便(trip)の経路探索や所要時間を提供する際、データ利用者は Zone2 を考慮に入れるべきではありません。

### ゾーンの重複制約 {: #zone-overlap-constraint}

同一の `trip_id` を持つ複数の stop_times.txt レコード間で、  
locations.geojson の `id` ジオメトリ、`start/end_pickup_drop_off_window` の時間、そして `pickup_type` または `drop_off_type` が同時に重複することは許可されていません。
